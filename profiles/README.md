...
- **Cross-root completeness.** `require_resource` only sees one root's plan, so it names what a root must create *itself*. A capability a workload *consumes* from another root (a platform-provided identity, a shared Key Vault) is out of view; proving that needs the descriptor's `provides`/`requires` graph, which ADR 0025 defers. Relationship rules (diagnostics-per-resource, private-endpoint-per-resource) are deferred for the same reason.
