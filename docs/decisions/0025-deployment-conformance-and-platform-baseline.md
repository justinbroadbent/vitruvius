...
- **Cross-root completeness.** The reference evaluator checks completeness against *one* root's plan (`require_resource` / `forbid_resource`), so it can name only what a root must create itself. Proving that a capability a workload *consumes* from another root (a platform-provided identity, a shared store) is present needs the descriptor's `provides`/`requires` graph, which this ADR leaves out (§"capabilities" was deliberately dropped). The §5 rule that the plan's tags must not contradict the descriptor's classification now ships (`tags_match_descriptor` — the resource group carries the exact values, direct-tag-required types fail closed, contradictions are caught, data sources are excluded, and the vocabulary is single-sourced from `modules/foundation/tags/vocabularies.yaml`). Relationship rules (diagnostics-per-resource, a private endpoint per data resource) remain follow-ups, deferred until a concrete workload triggers the first one.
