...
- `apply` — the step that makes real changes — runs after merge, per environment, behind an approval gate. A production apply requires an approver who is not the author. Within one pipeline run, apply consumes the exact saved plan that was gated and approved — bound by the plan hash recorded at the gate and re-verified before apply (§3), never a silent re-plan — so what ships is what was proven.
- Promotion is dev → staging → prod, each its own environment-subscription ([ADR 0024](./0024-landing-zone-binding-and-scope-vocabulary.md)). The same **code and descriptor** promote through every environment; because a Terraform plan is bound to one state and input set, each environment plans, gates, and applies its own plan — a dev plan is never applied to prod.
...
Each deployment emits an append-only **ledger** record — a log entry that can be added to but never edited: the PR link, commit SHA, plan/artifact hash, target environment and scope, executor identity, approver, timestamp, and outcome. The ledger is generated from pipeline state — never maintained by hand — and is durable and queryable. The reference slice ([`pipelines/`](../../pipelines/)) emits the per-deployment **receipt** carrying those fields; the durable, queryable, append-only ledger *service* that aggregates receipts across deployments remains a planned build. It serves double duty: the audit record, and the source of the DORA delivery signals ([ADR 0013](./0013-platform-metrics-and-dora.md)).
