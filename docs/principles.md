...
**Today**, CI runs `terraform fmt -check`, `terraform validate`, and `terraform test` per module and example (the module/example lists are discovered from the filesystem, so new ones cannot merge uncovered), an ADR-index drift check, and manifest validation — every `manifest.yaml` is checked against `schemas/module-manifest.schema.json` and cross-checked against the module's actual code for parity (ADR 0011), every `policy/*.json` is syntax-checked, every `catalog-info.yaml` is regenerated from its manifest with a drift gate (ADR 0016), the compliance control map is regenerated from its declared mappings with the same gate (ADR 0021), the implementation-status report is regenerated from its source with a drift gate that also fails if an accepted ADR has no status entry, and the conformance evaluator runs its self-test against plan fixtures (ADR 0025). That is the full set of automated gates currently wired — see the pipeline definition; [`IMPLEMENTATION-STATUS.md`](IMPLEMENTATION-STATUS.md) is the generated per-decision view of what is built versus planned.
