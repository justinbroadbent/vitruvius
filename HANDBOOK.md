...
- **The conformance gate is self-tested** — the ADR 0025 plan-policy evaluator runs against plan fixtures on every push: it catches a *missing* required capability (no workload identity), a forbidden one (a static client secret), a wrong property (a public Key Vault), and a property that is *missing or unknown* (it fails closed), and it waives a rule only on a valid, unexpired, rule-matched exemption. That proves the gate's logic; feeding it a *real* rendered plan on every PR waits on the deployment pipeline.
